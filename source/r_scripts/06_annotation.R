...
if (length(args) < 1) stop("Usage: Rscript 06_annotation.R <params.json>")
...
action <- as.character(params$action %||% "find_markers")
...
if (!file.exists(input_rds)) {
  write_error_summary("Annotation", paste("Input object not found:", input_rds), "Please make sure clustered.rds / annotated.rds exists before annotation.", output_dir)
  quit(status = 1)
}
...
set_group_order <- function(obj, params) {
  ...
  if (!is.null(group_order) && length(group_order) > 0 && "group" %in% colnames(obj@meta.data)) {
    group_order <- as.character(unlist(group_order))
    obj$group <- factor(obj$group, levels = group_order)
  }
  obj
}
...
build_composition_df <- function(group_vec, label_vec, group_order = NULL, label_col = "cell.type") {
  meta_df <- data.frame(
  ...
  if (nrow(meta_df) == 0) {
    stop("No valid group information was found for main composition plot.")
  }
  ...
  if (nrow(bad_df) > 0) {
    stop(
      paste0(
        "Main composition percent validation failed: ",
...
build_marker_bubble_plot_variant <- function(
  ...
  p_strip <- ggplot(label_display_df, aes(x = 1, y = label, fill = label)) +
    ...
    scale_y_discrete(drop = FALSE) +
    coord_cartesian(xlim = c(0.88, 1.12), clip = "off") +
    theme_void() +
...
build_marker_bubble_plot_dual <- function(obj, markers_csv, label_col, output_dir) {
  ...
  result$compact_fig <- tryCatch(
    build_marker_bubble_plot_variant(
      ...
      label_col = label_col,
      plot_basename = "marker_bubble_plot_compact",
      plot_title = "Main Marker Bubble Plot (Compact)",
    ...
    error = function(e) {
      result$message <<- paste0("Compact marker bubble plot failed: ", conditionMessage(e))
      NULL
    }
  )
  ...
  result$full_fig <- tryCatch(
    build_marker_bubble_plot_variant(
      ...
      markers_csv = markers_csv,
      label_col = label_col,
      plot_basename = "marker_bubble_plot_full",
      plot_title = "Main Marker Bubble Plot (Full)",
      output_dir = output_dir,
      top_n = Inf,
      width_cap = Inf,
      height_cap = Inf,
    ...
    error = function(e) {
      msg <- paste0("Full marker bubble plot failed but compact output was kept: ", conditionMessage(e))
...
run_scina_mapping <- function(obj, marker_obj, output_dir) {
  ...
  if (length(signatures) < 2) {
    stop("SCINA requires at least two cell types with markers found in the current object.")
  }
  ...
  scina_res <- SCINA::SCINA(
    ...
    rm_overlap = FALSE,
    allow_unknown = TRUE,
    log_file = tempfile(pattern = "scina_", fileext = ".log")
  )
  ...
  write.csv(pred_df, file.path(output_dir, "scina_cell_predictions.csv"), row.names = FALSE)
  write.csv(
    data.frame(cluster = names(mapping), label = unlist(mapping), stringsAsFactors = FALSE),
    file.path(output_dir, "scina_mapping.csv"),
    row.names = FALSE
  )
...
run_cellassign_mapping <- function(obj, marker_obj, output_dir) {
  ...
  if (is.null(marker_mat) || ncol(marker_mat) < 2) {
    stop("CellAssign requires at least two cell types with markers found in the current object.")
  }
  ...
  write.csv(pred_df, file.path(output_dir, "cellassign_cell_predictions.csv"), row.names = FALSE)
  write.csv(
    data.frame(cluster = names(mapping), label = unlist(mapping), stringsAsFactors = FALSE),
    file.path(output_dir, "cellassign_mapping.csv"),
    row.names = FALSE
  )
...
build_manual_marker_dotplot_variant <- function(
  ...
  if (length(marker_dict) == 0 || !label_col %in% colnames(obj@meta.data)) {
    return(list(fig = NULL, message = "Manual marker dotplot was skipped because no manual marker panel is available."))
  }
  ...
  if (length(gene_order) < 2) {
    return(list(fig = NULL, message = "Manual marker dotplot was skipped because fewer than two manual markers were found in the current object."))
  }
  ...
  dot_df <- dot_df[dot_df$features.plot %in% gene_order, , drop = FALSE]
  if (nrow(dot_df) == 0) {
    return(list(fig = NULL, message = "Manual marker dotplot was skipped because no expression values were available for the manual marker panel."))
  }
...
build_manual_marker_dotplot_dual <- function(obj, marker_dict, label_col, output_dir) {
  ...
  compact_res <- tryCatch(
    build_manual_marker_dotplot_variant(
      ...
      label_col = label_col,
      plot_basename = "manual_marker_dotplot_compact",
      plot_title = "Manual Marker DotPlot (Compact)",
      output_dir = output_dir,
    ...
    error = function(e) {
      list(fig = NULL, message = paste0("Compact manual marker dotplot failed: ", conditionMessage(e)))
    }
  )
  ...
  full_res <- tryCatch(
    build_manual_marker_dotplot_variant(
      ...
      marker_dict = marker_dict,
      label_col = label_col,
      plot_basename = "manual_marker_dotplot_full",
      plot_title = "Manual Marker DotPlot (Full)",
      output_dir = output_dir,
      max_genes_per_panel = Inf,
      width_cap = Inf,
      height_cap = Inf,
    ...
    error = function(e) {
      list(fig = NULL, message = paste0("Full manual marker dotplot failed but compact output was kept: ", conditionMessage(e)))
    }
  )
...
load_celldex_reference <- function(ref_id) {
  if (!requireNamespace("SingleR", quietly = TRUE)) {
    stop("SingleR package is not available. Please install it in R with BiocManager::install('SingleR')")
  }
  if (!requireNamespace("celldex", quietly = TRUE)) {
    stop("celldex package is not available. Please install it in R with BiocManager::install('celldex')")
  }
  ...
  if (!ref_id %in% names(ref_map)) {
    stop(sprintf("Unsupported celldex reference: %s", ref_id))
  }

  artifact_cache_root <- normalizePath(file.path(output_dir, "_gypsum_cache"), winslash = "/", mustWork = FALSE)
  runtime_gypsum_root <- file.path(artifact_cache_root, "ArtifactDB", "gypsum", "Cache")
  seed_celldex_cache(params$reference_cache_dir, runtime_gypsum_root)
  bucket_dir <- file.path(runtime_gypsum_root, "bucket", "celldex")
  status_dir <- file.path(runtime_gypsum_root, "status", "celldex")
  ...
  if (requireNamespace("gypsum", quietly = TRUE)) {
    previous_cache_dir <- gypsum::cacheDirectory()
    gypsum::cacheDirectory(normalizePath(runtime_gypsum_root, winslash = "/", mustWork = FALSE))
    on.exit(gypsum::cacheDirectory(previous_cache_dir), add = TRUE)
  }
  ...
  tryCatch(
    ...
    error = function(e) {
      ...
      if (grepl("Could not connect to server|HTTP request|gypsum.artifactdb.com", msg, ignore.case = TRUE)) {
        stop(
          paste0(
            "Failed to load celldex reference: ", ref_id, "\n",
            "The reference server is unavailable or the local cache is incomplete.\n",
            "Please preload the reference cache or retry the first download with network access.\n",
            "Details: ", msg
          )
        )
      }
      stop(paste0(
        "Failed to load celldex reference: ", ref_id, "\n",
        "Please check the local reference cache, celldex installation, and reference name.\n",
        "Details: ", msg
      ))
    }
  )
...
get_reference_labels <- function(ref) {
  if (!requireNamespace("SummarizedExperiment", quietly = TRUE)) {
    stop("celldex reference object is missing SummarizedExperiment support.")
  }
  cd <- as.data.frame(SummarizedExperiment::colData(ref))
  candidates <- c("label.main", "label.fine", "label.ont", "label")
  for (candidate in candidates) {
    if (candidate %in% colnames(cd)) {
      labels <- as.character(cd[[candidate]])
      labels[is.na(labels) | trimws(labels) == ""] <- "Unknown"
      non_unknown <- labels[labels != "Unknown"]
      mostly_cluster_ids <- length(non_unknown) > 0 && mean(grepl("^(cluster[_ -]?)?[0-9]+$", non_unknown, ignore.case = TRUE)) > 0.8
      if (length(unique(non_unknown)) > 1 && !mostly_cluster_ids) {
        cat(sprintf("SingleR reference label column: %s\n", candidate))
        return(labels)
      }
    }
  }
  stop("No usable non-cluster label column was found in the celldex reference object.")
}

seed_celldex_cache <- function(reference_cache_dir, runtime_cache_root) {
  bundled_root <- trimws(as.character(reference_cache_dir %||% ""))
  source_roots <- character(0)

  local_cache <- tryCatch(
    rappdirs::user_cache_dir("gypsum", "ArtifactDB"),
    error = function(e) ""
  )
  local_cache <- trimws(as.character(local_cache %||% ""))
  if (nzchar(local_cache) && dir.exists(local_cache)) {
    source_roots <- c(source_roots, local_cache)
  }

  if (nzchar(bundled_root)) {
    bundled_candidates <- c(
      bundled_root,
      file.path(bundled_root, "ArtifactDB", "gypsum", "Cache"),
      file.path(bundled_root, "R", "ArtifactDB", "gypsum", "Cache")
    )
    bundled_candidates <- unique(normalizePath(bundled_candidates, winslash = "/", mustWork = FALSE))
    for (candidate in bundled_candidates) {
      if (
        dir.exists(candidate) &&
        dir.exists(file.path(candidate, "bucket", "celldex")) &&
        dir.exists(file.path(candidate, "status", "celldex"))
      ) {
        source_roots <- c(source_roots, candidate)
      }
    }
  }

  source_roots <- unique(source_roots[file.exists(source_roots)])
  if (length(source_roots) == 0) {
    return(invisible(FALSE))
  }

  merge_cache_tree <- function(src_root, dst_root) {
    entries <- list.files(src_root, all.files = TRUE, no.. = TRUE, recursive = TRUE, full.names = TRUE)
    if (length(entries) == 0) {
      return(invisible(FALSE))
    }
    copied_any <- FALSE
    for (src in entries) {
      rel <- substring(src, nchar(src_root) + 2L)
      if (!nzchar(rel)) {
        next
      }
      target <- file.path(dst_root, rel)
      info <- file.info(src)
      if (isTRUE(info$isdir)) {
        ...
        if (!file.exists(target)) {
          ...
          if (!isTRUE(ok)) {
            stop(sprintf("Failed to seed celldex cache from %s", src))
          }
          copied_any <- TRUE
        }
      }
    }
    invisible(copied_any)
  }

  dir.create(runtime_cache_root, recursive = TRUE, showWarnings = FALSE)
  copied_any <- FALSE
  for (src_root in source_roots) {
    copied_any <- isTRUE(merge_cache_tree(src_root, runtime_cache_root)) || copied_any
  }

  lock_files <- list.files(runtime_cache_root, pattern = "^LOCK$", recursive = TRUE, full.names = TRUE)
...
validate_species_match <- function(species, ref_id) {
  species <- tolower(as.character(species %||% ""))
  human_refs <- c("HumanPrimaryCellAtlas", "BlueprintEncode", "MonacoImmune", "NovershternHematopoietic", "DatabaseImmuneCellExpression")
  mouse_refs <- c("MouseRNAseq", "ImmGen")
  if (species == "human" && ref_id %in% mouse_refs) {
    stop("Species is set to Human, but the selected reference is a Mouse reference.")
  }
  if (species == "mouse" && ref_id %in% human_refs) {
    stop("Species is set to Mouse, but the selected reference is a Human reference.")
  }
}
...
tryCatch({
  ...
  if (action == "find_markers") {
    ...
    report_progress(15, "Running FindAllMarkers...")
    markers <- FindAllMarkers(
      obj,
      min.pct = as.numeric(params$min_pct %||% 0.25),
      logfc.threshold = as.numeric(params$logfc_threshold %||% 0.25),
      only.pos = as.logical(params$only_pos %||% TRUE),
      test.use = as.character(params$test_use %||% "wilcox"),
      verbose = FALSE
    )
    ...
    all_tables <- c(all_tables, "all_markers.csv")

    fc_col <- get_fc_col(markers)
    top_markers <- markers %>% group_by(cluster) %>% slice_max(order_by = .data[[fc_col]], n = 10) %>% ungroup()
    write.csv(top_markers, file.path(output_dir, "top_markers.csv"), row.names = FALSE)
    all_tables <- c(all_tables, "top_markers.csv")

    top5 <- markers %>% group_by(cluster) %>% slice_max(order_by = .data[[fc_col]], n = 5) %>% ungroup()
    if (nrow(top5) > 0) {
      report_progress(70, "Generating marker heatmap...")
      p <- DoHeatmap(obj, features = unique(top5$gene), group.colors = sc_colors, size = 3) +
        theme(axis.text.y = element_text(size = 6))
      fig <- save_plot(p, "marker_heatmap", output_dir, width = 14, height = 10)
    ...
    report_progress(100, "Main markers finished")
  }

  if (action == "singler_annotate") {
    ref_id <- as.character(params$singler_ref_id %||% "HumanPrimaryCellAtlas")
    validate_species_match(params$species, ref_id)

    report_progress(10, "Preparing SingleR reference cache...")
    report_progress(25, "Loading celldex reference...")
    ref <- load_celldex_reference(ref_id)
    ref_labels <- get_reference_labels(ref)

    report_progress(55, "Running SingleR...")
    Idents(obj) <- "seurat_clusters"
    results <- SingleR::SingleR(
      test = get_rna_data(obj),
      ref = ref,
      labels = ref_labels,
      clusters = Idents(obj)
    )

    labels <- as.character(results$labels)
    if ("pruned.labels" %in% colnames(as.data.frame(results))) {
      pruned <- as.character(results$pruned.labels)
      labels[is.na(labels) | !nzchar(trimws(labels)) | grepl("^(cluster[_ -]?)?[0-9]+$", labels, ignore.case = TRUE)] <- pruned[is.na(labels) | !nzchar(trimws(labels)) | grepl("^(cluster[_ -]?)?[0-9]+$", labels, ignore.case = TRUE)]
    }
    labels[is.na(labels) | !nzchar(trimws(labels))] <- "Unknown"
    mapping <- setNames(labels, rownames(results))
    mapping_df <- data.frame(
      cluster = rownames(results),
      cell_type = labels,
      stringsAsFactors = FALSE
    )
    singler_cell_df <- data.frame(
      cell_id = colnames(obj),
      cluster = as.character(Idents(obj)),
      predicted_cell_type = unname(mapping[as.character(Idents(obj))]),
      stringsAsFactors = FALSE
    )
    singler_cell_df$predicted_cell_type[is.na(singler_cell_df$predicted_cell_type)] <- "Unknown"
    write.csv(singler_cell_df, file.path(output_dir, "singler_cell_predictions.csv"), row.names = FALSE)
    mapping_csv <- file.path(output_dir, "singler_mapping.csv")
    write.csv(mapping_df, mapping_csv, row.names = FALSE)
    all_tables <- c(all_tables, basename(mapping_csv), "singler_cell_predictions.csv")

    report_progress(82, "Saving SingleR mapping...")
    write_summary(list(
      status = "success",
      action = "singler_annotate",
      reference_id = ref_id,
      singler_mapping = as.list(mapping),
      cluster_ids = as.list(rownames(results)),
      figures = list(),
      tables = as.list(all_tables)
    ), output_dir)
    report_progress(100, "SingleR annotation finished")
  }
  ...
  if (action == "scina_annotate") {
    ...
    all_tables <- c(all_tables, "scina_mapping.csv", "scina_cell_predictions.csv")
    write_summary(list(
      status = "success",
    ...
    report_progress(100, "SCINA annotation finished")
  }
  ...
  if (action == "cellassign_annotate") {
    ...
    all_tables <- c(all_tables, "cellassign_mapping.csv", "cellassign_cell_predictions.csv")
    write_summary(list(
      status = "success",
    ...
    report_progress(100, "CellAssign annotation finished")
  }

  if (action == "apply_annotation") {
    mapping <- params$cluster_mapping
    if (is.null(mapping) || length(mapping) == 0) {
      stop("Cluster-to-celltype mapping is missing.")
    }
    report_progress(10, "Applying cell type mapping...")
    Idents(obj) <- "seurat_clusters"
    cell_types <- unlist(mapping)
    names(cell_types) <- names(mapping)
    current_idents <- as.character(Idents(obj))
    cell_type_vec <- cell_types[current_idents]
    cell_type_vec[is.na(cell_type_vec)] <- "Unknown"
    names(cell_type_vec) <- colnames(obj)
    obj@meta.data$cell.type <- sanitize_label_vec(cell_type_vec, fallback = "Unknown")

    report_progress(75, "Saving annotated object...")
    saveRDS(obj, file.path(output_dir, "annotated.rds"))
    ct_list <- sort(unique(obj$cell.type))
    writeLines(ct_list, file.path(output_dir, "cell_types.txt"))
    write_summary(list(
      status = "success",
      action = "apply_annotation",
      cell_types = as.list(ct_list),
      output_rds = "annotated.rds",
      figures = list(),
      tables = list()
    ), output_dir)
    report_progress(100, "Main annotation finished")
  }
  ...
  if (action == "generate_plots") {
    ...
    if (!red %in% Reductions(obj)) {
      stop(sprintf("Requested reduction '%s' was not found. Please regenerate clustering reductions first.", red))
    }

    red_label <- if (identical(red, "tsne")) "t-SNE" else toupper(red)
    report_progress(10, paste0("Generating main annotation plot (", red_label, ")..."))
    p1 <- DimPlot(obj, reduction = red, group.by = "cell.type", cols = sc_colors, label = TRUE, label.size = 5.25, repel = TRUE) +
      labs(title = paste0("Main Annotation (", red_label, ")")) + sc_theme
    fig <- save_plot(p1, paste0(red, "_celltype"), output_dir, width = 10, height = 8)
    all_figures <- c(all_figures, basename(fig))

    n_groups <- length(unique(obj$group))
    if (n_groups > 1 && n_groups <= 6) {
      report_progress(35, "Generating group split plot...")
      p2 <- DimPlot(obj, reduction = red, group.by = "cell.type", split.by = "group", cols = sc_colors, label = TRUE, label.size = 3.5) +
        labs(title = paste0("Main Annotation by Group (", red_label, ")")) + sc_theme
      fig <- save_plot(p2, paste0(red, "_celltype_split"), output_dir, width = 6 * n_groups, height = 6)
      all_figures <- c(all_figures, basename(fig))
    }

    manual_marker_message <- ""
    if (n_groups > 1) {
      report_progress(65, "Calculating main composition...")
      group_order <- if (is.factor(obj$group)) levels(obj$group) else unique(as.character(obj$group))
      prop_df <- build_composition_df(
        group_vec = obj$group,
        label_vec = obj$cell.type,
      ...
      p_bar <- ggplot(prop_df, aes(x = x_pos, y = proportion, fill = cell.type)) +
        geom_col(width = 0.6) +
        ...
        theme_classic() + ylab("Proportion") + xlab("") +
        theme(plot.margin = margin(5.5, 14, 5.5, 14)) +
        ggtitle("Main Cell Type Composition") + sc_theme
      fig <- save_plot(p_bar, "composition_bar", output_dir, width = plot_width, height = 6)
      all_figures <- c(all_figures, basename(fig))
      comp_csv <- file.path(output_dir, "composition_main_percent.csv")
      write.csv(prop_df, comp_csv, row.names = FALSE)
      all_tables <- c(all_tables, basename(comp_csv))
    }

    report_progress(88, "Generating marker bubble plot...")
    bubble_plot <- build_marker_bubble_plot_dual(
      obj = obj,
      markers_csv = file.path(output_dir, "top_markers.csv"),
      label_col = "cell.type",
      output_dir = output_dir
    )
    ...
    report_progress(100, "Main annotation plots finished")
  }
}, error = function(e) {
  err_msg <- conditionMessage(e)
  suggestion <- "Please check the input object, clustering results, and annotation parameters."
  if (grepl("SingleR|celldex|reference", err_msg, ignore.case = TRUE)) {
    suggestion <- paste0(
      "Please verify that SingleR / celldex is installed correctly and the reference download succeeded.\n",
      "If the packages are missing, run BiocManager::install(c('SingleR', 'celldex')) in R."

    )
  } else if (grepl("SCINA", err_msg, ignore.case = TRUE)) {
    suggestion <- paste0(
      "Please verify that the local R runtime contains the SCINA package and that each cell type has markers present in the current object.\n",
  ...
  write_error_summary("Annotation", err_msg, suggestion, output_dir)
  quit(status = 1)
})
